<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#fff","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px 0px 20px 20px","margin":"20px 0 0","overflow":"hidden","borderRadius":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0 20px","borderColor":"#2a8ba7","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","borderWidth":"0px 0 0","width":"auto","fontSize":"inherit","borderStyle":"solid","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="离场状态" prop="lichangzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">离场状态</label>
						<el-select clearable v-model="searchForm.lichangzhuangtai" placeholder="请选择离场状态" >
							<el-option v-for="(item,index) in lichangzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 20px","margin":"0px 0 0","borderColor":"#ddd","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"0px","flexWrap":"wrap","background":"none","borderWidth":"0px 0 0","width":"auto","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('daochangxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('daochangxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0 0px 0 20px","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"rgba(255,255,255,0)","borderWidth":"0px 0 0","width":"calc(100% - 20px)","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="daochangshijian"
						label="到场时间">
						<template slot-scope="scope">
							{{scope.row.daochangshijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lichangzhuangtai"
						label="离场状态">
						<template slot-scope="scope">
							{{scope.row.lichangzhuangtai}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('daochangxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('daochangxinxi','离场签退')" @click="lichangxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','lichangzhuangtai','已离场','已离场,待离场'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								离场签退
							</el-button>
							<el-button class="edit" v-if=" isAuth('daochangxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('daochangxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<lichangxinxi-cross-add-or-update v-if="lichangxinxiCrossAddOrUpdateFlag" :parent="this" ref="lichangxinxiCrossaddOrUpdate"></lichangxinxi-cross-add-or-update>
...
import lichangxinxiCrossAddOrUpdate from "../lichangxinxi/add-or-update";
	export default {
		data() {
			return {
				lichangzhuangtaiOptions: [],
				searchForm: {
					key: ""
				},
				form:{},
				...
				lichangxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			lichangxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.lichangxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','daochangxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.lichangxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.lichangxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          this.lichangzhuangtaiOptions = "已离场,待离场".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.lichangzhuangtai!='' && this.searchForm.lichangzhuangtai!=undefined){
            params['lichangzhuangtai'] = this.searchForm.lichangzhuangtai
          }
			let user = JSON.parse(this.$storage.getObj('userForm'))
			console.log(user)
			this.$http({
				url: "daochangxinxi/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
	...
	daochangxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'daochangxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
