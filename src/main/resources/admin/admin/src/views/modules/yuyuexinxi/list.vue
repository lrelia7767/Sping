<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#fff","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px 0px 20px 20px","margin":"20px 0 0","overflow":"hidden","borderRadius":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0 20px","borderColor":"#2a8ba7","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","borderWidth":"0px 0 0","width":"auto","fontSize":"inherit","borderStyle":"solid","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">分类</label>
						<el-input v-model="searchForm.fenlei" placeholder="分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">地址</label>
					...
					<div :style='{"margin":"0 10px 10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="到场状态" prop="daochangzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">到场状态</label>
						<el-select clearable v-model="searchForm.daochangzhuangtai" placeholder="请选择到场状态" >
							<el-option v-for="(item,index) in daochangzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">是否通过</label>
				...
				<el-row class="actions" :style='{"padding":"0 20px","margin":"0px 0 0","borderColor":"#ddd","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"0px","flexWrap":"wrap","background":"none","borderWidth":"0px 0 0","width":"auto","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('yuyuexinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yuyuexinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuyuexinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						审核
					</el-button>

					<el-button class="btn18" v-if="isAuth('yuyuexinxi','每日预约人数统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0 0px 0 20px","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"rgba(255,255,255,0)","borderWidth":"0px 0 0","width":"calc(100% - 20px)","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="daochangzhuangtai"
						label="到场状态">
						<template slot-scope="scope">
							{{scope.row.daochangzhuangtai}}
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyuexinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyuexinxi','到场签到')" @click="daochangxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','daochangzhuangtai','已签到','已到场,未到场'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								到场签到
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyuexinxi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyuexinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<daochangxinxi-cross-add-or-update v-if="daochangxinxiCrossAddOrUpdateFlag" :parent="this" ref="daochangxinxiCrossaddOrUpdate"></daochangxinxi-cross-add-or-update>
...
import daochangxinxiCrossAddOrUpdate from "../daochangxinxi/add-or-update";
	export default {
		data() {
			return {
				...
				daochangxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			daochangxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.daochangxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.daochangxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.daochangxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "yuyuexinxi/group/shenqingriqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.daochangzhuangtai!='' && this.searchForm.daochangzhuangtai!=undefined){
            params['daochangzhuangtai'] = this.searchForm.daochangzhuangtai
          }
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "yuyuexinxi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
