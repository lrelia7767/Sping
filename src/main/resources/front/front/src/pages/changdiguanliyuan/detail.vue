...
<div>
	<div :style='{"padding":"20px 0","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"none","borderWidth":"0 0 0px","width":"64%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'='" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/changdiguanliyuan"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"51%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"minHeight":"460px","padding":"20px","margin":"0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 0px","margin":"0 0 4px 0","borderColor":"#28890b10","alignItems":"center","background":"#fff","borderWidth":"0px","display":"flex","width":"100%","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"right","width":"auto","fontSize":"15px","lineHeight":"32px","height":"32px"}'>管理账号</div>
					<div  :style='{"padding":"5px 10px 0","fontSize":"15px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.guanlizhanghao}}</div>
				</div>
				<div class="item" :style='{"padding":"0 0px","margin":"0 0 4px 0","borderColor":"#28890b10","alignItems":"center","background":"#fff","borderWidth":"0px","display":"flex","width":"100%","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"right","width":"auto","fontSize":"15px","lineHeight":"32px","height":"32px"}'>管理姓名</div>
					<div  :style='{"padding":"5px 10px 0","fontSize":"15px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.guanlixingming}}</div>
				</div>
				<div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0px solid #009cf530","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#009cf5","borderRadius":"0px","background":"none","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('changdiguanliyuan','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#f53400","borderRadius":"0px","background":"none","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('changdiguanliyuan','删除')" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/changdiguanliyuan', query: params});
		},
		// 下载
		download(file){
			if(!file) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/changdiguanliyuanAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此场地管理员？')
			  .then(_ => {
			    this.$http.post('changdiguanliyuan/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
