...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>分类：</div>
        <el-input v-model="formSearch.fenlei" placeholder="分类" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	  ...
	  <el-button v-if="btnAuth('daochangxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","display":"inline-block","outline":"none","borderRadius":"20px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/daochangxinxiAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","width":"calc(100% - 250px)","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 0px","fontSize":"14px","color":"#666","background":"none","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 10px","margin":"0 2% 20px 0","overflow":"hidden","background":"#fff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.tupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.tupian?item.tupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.mingcheng}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">地址:{{item.dizhi}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.daochangshijian}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">离场状态:{{item.lichangzhuangtai}}</div>
...
<script>
  export default {
    ...
    async created() {
		...
		this.selectOptionsList.push({name:'离场状态',list:this.lichangzhuangtaiOptions,tableName: 'lichangzhuangtai',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.fenlei != '') searchWhere.fenlei = '%' + this.formSearch.fenlei + '%';
        ...
        if (this.formSearch.lichangzhuangtai != '') searchWhere.lichangzhuangtai = this.formSearch.lichangzhuangtai;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`daochangxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/daochangxinxiDetail', query: params});
      },
